...
public class tree : MonoBehaviour
{
    ...
    {
        if (Random.Range(0.0f, 1.0f) < threshold) return true;
        return false;
    }

    [SerializeField]
    public PlantType type = PlantType.ALTERNATE;
    ...
    [SerializeField]
    private GameObject branchPrefab = null;

    [SerializeField, Range(10.0f, 90.0f)]
    private float minSplitAngle = 30.0f;

    [SerializeField, Range(10.0f, 90.0f)]
    private float maxSplitAngle = 40.0f;
    ...
    [SerializeField, Range(0.0f, 10.0f)]
    public float splitThresholdOne = 1.0f;

    [SerializeField, Range(0.0f, 10.0f)]
    public float splitThresholdTwo = 2.0f;

    [SerializeField, Range(0.0f, 10.0f)]
    public float splitThresholdThree = 5.0f;

    [SerializeField, Range(0.0f, 10.0f)]
    public float splitThresholdFour = 6.0f;

    [SerializeField, Range(0.0f, 0.0001f)]
    public float curveAmount = 0.0001f;
    ...
    void Update()
    {
        ...
        if (height > 2 && !switched) curveDirection = -curveDirection;
        ...
        if (!(last < Globals.distanceTravelled)) return;
        last = Globals.distanceTravelled;

        height += 0.35f * delta;

        if (multiplier > 0.05f)
            multiplier = clamp(multiplier - 0.005f * delta, 0.0f, 2.0f);
        ...
        transform.localScale =
            new Vector3(height * leafSizeMult,
                height * leafSizeMult,
    ...
    public void Split(bool right)
    {
        ...
        GameObject newBranch =
            Instantiate(branchPrefab, transform.position, transform.rotation);
